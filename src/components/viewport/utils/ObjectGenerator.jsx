...
;
  ...
  const getMaterial = (defaultType = AssetsManager.MATERIAL_TYPE.MESH_STANDARD_MATERIAL) => {
    if (sharedMaterial) {
      return sharedMaterial;
    }
    return assetsManager.createNewMaterial(defaultType);
  };

  const setSharedMaterial = (material) => {
    setSharedMaterial(material);
  };

  const unsetSharedMaterial = () => {
    setSharedMaterial(null);
  };

  return {
    addPlane: (material = getMaterial(), attachProperties = true) => {
      const object = createPlane(material, attachProperties);
      viewport.add(object);
      return object;
    },

    createPlane: (material = getMaterial(), attachProperties = true) => {
      const geometry = new THREE.PlaneGeometry(1, 1);
      material.side = THREE.DoubleSide;
      return createInteractiveObject(geometry, material, attachProperties, PlaneProperty);
    },

    addCube: (material = getMaterial(), attachProperties = true) => {
      const object = createCube(material, attachProperties);
      viewport.add(object);
      return object;
    },

    createCube: (material = getMaterial(), attachProperties = true) => {
      const geometry = new THREE.BoxGeometry(1, 1, 1);
      return createInteractiveObject(geometry, material, attachProperties, BoxProperty);
    },

    addCircle: (material = getMaterial(), attachProperties = true) => {
      const object = createCircle(material, attachProperties);
      viewport.add(object);
      return object;
    },

    createCircle: (material = getMaterial(), attachProperties = true) => {
      const geometry = new THREE.CircleGeometry(1, 10);
      return createInteractiveObject(geometry, material, attachProperties, CircleProperty);
    },

    addUVSphere: (material = getMaterial(), attachProperties = true) => {
      const object = createUVSphere(material, attachProperties);
      viewport.add(object);
      return object;
    },

    createUVSphere: (material = getMaterial(), attachProperties = true) => {
      const geometry = new THREE.SphereGeometry(1, 30, 30);
      return createInteractiveObject(geometry, material, attachProperties, SphereProperty);
    },

    addIcoSphere: (material = getMaterial(), attachProperties = true) => {
      const object = createIcoSphere(material, attachProperties);
      viewport.add(object);
      return object;
    },

    createIcoSphere: (material = getMaterial(), attachProperties = true) => {
      const geometry = new THREE.IcosahedronGeometry(1, 2);
      return createInteractiveObject(geometry, material, attachProperties, IcosahedronProperty);
    },

    addCylinder: (material = getMaterial(), attachProperties = true) => {
      const object = createCylinder(material, attachProperties);
      viewport.add(object);
      return object;
    },
